...
# PWT 9.0 のデータを読み込み
# インターネット上のURLからデータを読み込もうとしています。
pwt90 = pd.read_stata('https://www.rug.nl/ggdc/docs/pwt90.dta')

# 分析対象のOECD国を定義 (リストを拡張)
oecd_countries = [
...
# データをフィルタリング: 対象国と指定された期間 (1960-2000年) のデータのみを抽出
data = pwt90[
    pwt90['country'].isin(oecd_countries) &
    pwt90['year'].between(1960, 2000)
]

# 関連する列のみを選択し、欠損値を含む行を削除
...
# 国ごとにデータをソートし、基準年を100として各変数をシフト（正規化）
...
def calculate_growth_rates(country_data):
    ...
    # 期間が1年以下の場合、成長率を計算できないためNoneを返す
    if years <= 0:
        return None
    ...
    # ゼロ除算を避けるためのチェック
    tfp_share = (tfp_growth_calculated / g_y) if g_y != 0 else np.nan
    cap_share = (capital_deepening_contrib / g_y) if g_y != 0 else np.nan
